...
async function scrapeBuiltinHouston(browser, config) {
  ...
  for (const job of jobs) {
    ...
    console.log(`  [builtin-houston] DEBUG blank company raw job:`, JSON.stringify(job, null, 2));
    const detailCompany = await fetchBuiltinCompanyFromDetail(browser, job.sourceUrl);
    if (detailCompany) {
      job.company = detailCompany;
      console.log(`  [builtin-houston] recovered company from detail page: ${detailCompany}`);
    } else {
      console.log(`  [builtin-houston] company still blank after detail-page fallback: ${job.sourceUrl}`);
    }
  }
